...
const RecipeCard = ({ recipe, refetch, userId }) => {
    ...
    return (
        <div className={`card recipeCard border-1 border-secondary-emphasis col-3 mx-4 my-2 ${recipe.img ? loaded ? 'fade-in' : 'visually-hidden' : 'fade-in'}`}>
            <div key={recipe._id} className='d-flex flex-column align-items-center' data-bs-toggle="modal" data-bs-target={`#RecipeModal-${recipe._id}`}>
                {recipe.img ? (<img className="img col-12" onLoad={() => setLoaded(true)} src={recipe.img} alt={recipe.name} />) : (null)}
                <h3 className="text-center my-2 fw-bold">{recipe.name}</h3>
            </div>
